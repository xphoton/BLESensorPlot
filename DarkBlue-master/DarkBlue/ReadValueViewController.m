...
//        NSString *s = [NSString stringWithUTF8String:[d bytes]];
        ...
        NSLog(@"read (%@):\n%@\n%u",d,hexInt,deciInt);
        self.lbHex.text = hexInt;
        self.lbDecimal.text = [NSString stringWithFormat:@"%u", deciInt];
    });
}
/*
...
- (IBAction)notify:(id)sender {
    if ([self.notifyButton.titleLabel.text  isEqual: @"Notify"]) {
        [self.defaultBTServer setNotificationYes:self.defaultBTServer.selectCharacteristic];
        [self.notifyButton setTitle:@"Stop" forState:UIControlStateNormal];
        [self readAction];
    }
    else if ([self.notifyButton.titleLabel.text  isEqual: @"Stop"]) {
        [self.defaultBTServer setNotificationNo:self.defaultBTServer.selectCharacteristic];
        [self.notifyButton setTitle:@"Notify" forState:UIControlStateNormal];
    }
}
